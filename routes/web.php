...
Route::group(['prefix' => 'app'],function () {
    ...
    Route::group(['middleware' => 'auth'], function () {
        Route::group(['prefix' => 'schedule'], function() {
            ...
            Route::get('/modal-schedule', [ScheduleController::class, 'modalSchedule'])->name('schedule.modal-schedule');
            Route::post('/to-schedule', [ScheduleController::class, 'store'])->name('schedule.store');
            Route::post('/', [ScheduleController::class, 'showSpecificShcedule'])->name('schedule.show-specific-shedule');
            Route::post('/to-destroy-schedule', [ScheduleController::class, 'destroy'])->name('schedule.destroy');
            Route::get('/user-schedules/{user_id?}', [ScheduleController::class, 'userSchedules'])->name('schedule.user-schedules');
            Route::get('/fechamento-mes/{user_id?}', [ScheduleController::class, 'fechamentosDoMes'])->name('schedule.fechamento-mes');
            Route::post('/mudar-tipo-agendamento', [ScheduleController::class, 'mudarTipoAgendamento'])->name('schedule.mudar-tipo-agendamento');
            Route::get('/update-all-schedules', [ScheduleController::class, 'updateAllSchedules'])->name('schedule.update-all-schedules');
            Route::get('/details', [ScheduleController::class, 'details'])->name('schedule.details');
            Route::post('/destroy-schedule-next-month', [ScheduleController::class, 'destroyNextMonth']);
            Route::post('/mudar-tipo-agendamento-proximo-mes', [ScheduleController::class, 'mudarTipoAgendamentoProximoMes']);
            Route::post('/cancel-all-fixed-schedules/{user_id?}', [ScheduleController::class, 'cancelAllFixedSchedules']);
            Route::post('/cancel-all-fixed-next-month-schedules/{user_id?}', [ScheduleController::class, 'cancelAllFixedNextMonthSchedules']);
            Route::get('/modal-cancelar-agendamento-fixo', [ScheduleController::class, 'modalCancelarAgendamentoFixo'])->name('schedule.modal-cancelar-agendamento-fixo');
            Route::post('/cancelar-agendamento-fixo', [ScheduleController::class, 'cancelarAgendamentoFixo'])->name('schedule.cancelar-agendamento-fixo');
            Route::get('/index-administrador', [ScheduleController::class, 'indexAdmin'])->name('schedule.index-administrador');
            Route::post('/index-administrador', [ScheduleController::class, 'showSpecificShceduleAdministrador'])->name('schedule.show-specific-shedule-administrador');
            Route::get('/schedule-search', [ScheduleController::class, 'scheduleSearch'])->name('schedule.search');
            Route::post('/schedule-search', [ScheduleController::class, 'showSpecificShceduleMonth'])->name('schedule.show-specific-shedule-month');
        });
