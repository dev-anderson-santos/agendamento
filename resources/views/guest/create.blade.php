                ...
                <form id="cadastro-profissional" class="form-group" method="post" action="{{ route('user.store') }}" autocomplete="off">
                    ...
                    <fieldset>
                        ...
                        <div class="row">                                        
                            ...
                            <div class="col-12 col-md-3 mt-2 mb-2">
                                <label class="form-labels" for="especialidade">Especialidade</label>
                                <select id="especialidade" name="state" class="form-control especialidade" required>
                                    <option value="">-- Selecione --</option>
                                    @foreach (getEspecialidade() as $key => $item)
                                        <option value="{{ $key }}" {{ !empty($user) && $user->address->state == $key ? 'selected' : (old('state') == $key ? 'selected' : '') }}>{{ $item }}</option>
                                    @endforeach
                                </select>
                            </div>                                        
                        </div>
                    ...
                    <button type="submit" class="btn btn-primary btn-small btn_clientes float-right">Enviar Cadastro</button>
                </form>
            </div>
        </div>
    </div>
